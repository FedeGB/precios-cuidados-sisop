...
{	
	if [[ "$2" == "-1" ]]; then # Verifico si existe el Supermercado
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por Supermercado inexistente" "ERR";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$1 a $GRUPO/$RECHDIR/$1";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$1" "$GRUPO/$RECHDIR/$1" "Masterlist";
		return 2;
	fi

	if [[ $3 -eq -1 ]]; then # Verifico cantidad de campos
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por Cantidad de campos invalida" "ERR";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$1 a $GRUPO/$RECHDIR/$1";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$1" "$GRUPO/$RECHDIR/$1" "Masterlist";
		return 2;
	else
		if [[ $4 -eq -1 || $4 -gt $3 || $4 -eq $5 ]]; then # Verifico posicion producto
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por Posicion producto invalida" "ERR";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$1 a $GRUPO/$RECHDIR/$1";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$1" "$GRUPO/$RECHDIR/$1" "Masterlist";
			return 2;
		fi 					
		...
		if [[ $5 -eq -1 || $5 -gt $3 || $4 -eq $5 ]]; then # Verifico posicion precio
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por Posicion precio invalida" "ERR";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$1 a $GRUPO/$RECHDIR/$1";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$1" "$GRUPO/$RECHDIR/$1" "Masterlist";
			return 2;
		fi
	...
	if [[ "$6" == "-1" ]]; then # Verifico correo electronico
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por Correo electronico del colaborador invalido" "ERR";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$1 a $GRUPO/$RECHDIR/$1";
		bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$1" "$GRUPO/$RECHDIR/$1" "Masterlist";
		return 2;
	fi
...
function procesarArchivo
{
	...
	bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Archivo $1 ha sido procesado" "INF";
	bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Registros ok: $cantidadRegistrosOk" "INF";
	bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Registros nok: $cantidadRegistrosNok" "INF";
}
...
bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Inicio de Masterlist"
cantidadArchivos=`ls $pathPrecios | wc -l`
cantidadArchivos=`expr $cantidadArchivos - 1` #Resto uno por la carpeta proc
bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Cantidad de Listas de precios a procesar: $cantidadArchivos"
#Fin cabecera de log
...
if [[ $cantidadArchivos -eq 0 ]]; then
	...
	for archivoPrecios in $(ls $pathPrecios); do 
		...
		bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Archivo a procesar: $archivoPrecios"
		if [ -e $pathProcesados/$archivoPrecios ]; then #Archivo ya procesado
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por estar DUPLICADO" "ERR";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$archivoPrecios a $GRUPO/$RECHDIR/$archivoPrecios";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$archivoPrecios" "$GRUPO/$RECHDIR/$archivoPrecios" "Masterlist";
			continue;
		fi
		...
		if [[ "$fechaArchivo" == "-1" ]]; then # Archivo de lista de precios con fecha invalida, se rechaza.
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por FECHA INVALIDA" "ERR";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$archivoPrecios a $GRUPO/$RECHDIR/$archivoPrecios";
			bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$archivoPrecios" "$GRUPO/$RECHDIR/$archivoPrecios" "Masterlist";
			continue;
		fi	
		...
		if [[ -e $preciosMae ]]; then	
			...
			if ! [[ "$busquedaRegistro" == "-1" ]]; then
				...
				if [[ "$?" == "1" ]]; then
					...
					bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Registros eliminados: $cantidadRegistrosEliminados" "INF";
					bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$archivoPrecios a $pathProcesados/$archivoPrecios";
					bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$archivoPrecios" "$pathProcesados/$archivoPrecios" "Masterlist";			
				else 
					# La fecha del registro encontrado en precios.mae es mayor a la fecha del archido a procesar
					bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Se rechaza el archivo por fecha anterior a la existente" "ERR";
					bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Moviendo $pathPrecios/$archivoPrecios a $GRUPO/$RECHDIR/$archivoPrecios";
					bash /home/ezequiel/precios-cuidados-sisop/Tools/Mover.sh "$pathPrecios/$archivoPrecios" "$GRUPO/$RECHDIR/$archivoPrecios" "Masterlist";
				fi
			else
...
bash /home/ezequiel/precios-cuidados-sisop/Tools/logging.sh "Masterlist" "Fin de Masterlist"
